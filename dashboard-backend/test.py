...
def fetch_unique_accounts_with_balance_and_equity(db: Session):
    ...
    SELECT DISTINCT ON (flr.account_number)
        ...
        flr.export_time,
        flr.strategy_name,
        COALESCE(flr.balance - COALESCE(dsr.day_start_balance, fpdb.prev_day_balance), flr.balance) AS day_profit,
        COALESCE(flr.balance - COALESCE(wsr.week_start_balance, fpwb.prev_week_balance), flr.balance) AS week_profit, -- New line for week profit
        COALESCE(flr.balance - COALESCE(msr.month_start_balance, fpmb.prev_month_balance), flr.balance) AS month_profit, -- New line for month profit
    ...
    LEFT JOIN week_start_records wsr ON flr.account_number = wsr.account_number -- New line for week start balance
    LEFT JOIN filtered_previous_week_balance fpwb ON flr.account_number = fpwb.account_number -- New line for previous week balance
    LEFT JOIN month_start_records msr ON flr.account_number = msr.account_number -- New line for month start balance
    LEFT JOIN filtered_previous_month_balance fpmb ON flr.account_number = fpmb.account_number -- New line for previous month balance
